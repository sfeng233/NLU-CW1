...
class RNN(object):
	...
	def predict(self, x):
		...
		one_hot_enc = []
		for word in x:
			one_hot_enc.append(make_onehot(word,self.vocab_size))

		for t in range(len(x)):
			one_hot_enc = make_onehot(x[t], self.vocab_size)
			s[t] = sigmoid(np.dot(self.V, one_hot_enc)+np.dot(self.U, s[t-1])) 
	...
	def acc_deltas(self, x, d, y, s):
		...
		der_softmax = np.ones(self.vocab_size)

		for t in reversed(range(len(x))):
			dif_out = np.multiply((make_onehot(d[t], self.out_vocab_size) - y[t]), der_softmax)
			self.deltaW += np.outer(dif_out, s[t])
			der_sigmoid = np.multiply(s[t], (np.ones(len(s[t])) - s[t]))
			dif_in = np.multiply(np.dot(self.W.T, dif_out), der_sigmoid)
			self.deltaV += np.outer(dif_in,make_onehot(x[t], self.vocab_size))
			self.deltaU += np.outer(dif_in, s[t-1])
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			for i in range(1,steps+1):
				der_sigmoid = np.multiply(s[t-i], (np.ones(len(s[t-i])) - s[t-i]))
				dif_in = np.multiply(np.dot(self.U.T, dif_in), der_sigmoid)
				self.deltaV += np.outer(dif_in, make_onehot(x[t-i], self.vocab_size))
				self.deltaU += np.outer(dif_in, s[t-i-1])
	...
	def compute_mean_loss(self, X, D):
		...
		for i,sent in enumerate(X):
			total_loss += self.compute_loss(sent, D[i])
			word_count += len(sent)
